...
Listado.prototype.buscarRestaurante = function(id) {
  //cargar el restaurant
  var restaurant = this.restaurantes.find(restaurant => restaurant.id === id)
  
  //si se encontro el restaurant devolver el mismo de lo contrario devolver mensaje notificador
  return restaurant ? restaurant : "No se ha encontrado ningún restaurant";
}

var distinctOrderArray = (array) => array.filter((elemento, index, self) => index === self.indexOf(elemento)).sort()

//Obtiene todas las ciudades de los restaurantes sin repetidos
Listado.prototype.obtenerUbicaciones = function() {
  //Se devuelven las ubicaciones ordenadas y sin repetirse de todos los restaurantes
  return distinctOrderArray(this.restaurantes.map(restaurant => restaurant.ubicacion))
}
...
Listado.prototype.obtenerRubros = function() {
  //Se devuelven los rubros ordenados y sin repetirse de todos los restaurantes
  return distinctOrderArray(this.restaurantes.map(restaurant => restaurant.rubro))
}
...
Listado.prototype.obtenerHorarios = function() {
  ...
  //para cada restaurant y para cada horario de cada restaurant agregar horario a horarios
  this.restaurantes.forEach(function(restaurant) {
    restaurant.horarios.map(horario => horarios.push(horario))
  })

  //Se devuelven loss horarios ordenados y sin repetirse
  return distinctOrderArray(horarios)
}
